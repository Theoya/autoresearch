"""
Data preparation and evaluation for SQUAT form regression.

Copy of prepare.py adapted for the squat task:
  - DATA_DIR points at the claude squat labelled features
  - METRIC_NAMES = the 10 squat metric keys
  - NUM_OUTPUTS = 10

Each sample = 75 frames x 334 features + sex scalar -> 10 form metric scores (0-9).

Usage:
    python prepare_squat.py   # load data, print stats
"""
...
NUM_OUTPUTS = 10
DATA_DIR = "T:/clipforge/datasets/claude_squat_labelled"
TIME_BUDGET = 12000  # training time budget in seconds

METRIC_NAMES = [
    "stanceWidth", "squatDepth", "kneeTracking", "trunkAngle", "lumbarSpine",
    "thoracicPosture", "barPath", "descentControl", "hipDrive", "lockout",
]
...
def load_all_data():
    ...
    Returns:
        ...
        y: tensor (N, 10) - form metric scores
    ...
    y = torch.tensor(np.array(y_list), dtype=torch.float32)  # (N, 10)
...
def evaluate_rmse(predictions, targets):
    """
    Root Mean Squared Error across all 10 outputs jointly.
    """
