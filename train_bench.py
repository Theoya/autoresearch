"""
BENCH PRESS form regression training script.
Best-k ensemble of per-metric tiny MLPs - train many, keep the best - blended
with per-metric ExtraTrees. Mirror of train_squat.py importing prepare_bench (9 metrics).
Usage: uv run train_bench.py
"""
...
def load_all_data_cached():
    ...
    cache_path = os.path.join(cache_dir, f"bench_{len(files)}_{key}.pt")
...
BLEND_W = 0.5    # final = (1-W)*ExtraTrees + W*neural-ensemble
...
# ---------------------------------------------------------------------------
# Per-metric ExtraTrees + blend (same recipe as squat/deadlift).
# ---------------------------------------------------------------------------
